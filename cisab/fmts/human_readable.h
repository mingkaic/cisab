...
#include "cisab/fmts/istreamable.h"
...
template <typename IT>
void arr_to_stream (std::ostream& s, IT begin, IT end, std::string delim)
{
	if (begin != end)
	{
...
/// Stream pair p to s given specified delim between first and second elements
template <typename PL, typename PR>
void pair_to_stream (std::ostream& s, std::pair<PL,PR> p, std::string delim)
{
